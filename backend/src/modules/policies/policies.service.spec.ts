...
describe('PoliciesService', () => {
  ...
  describe('uploadDocx', () => {
    it('creates a new current version from the uploaded DOCX', async () => {
      ...
      (prisma.policyVersion.findFirst as jest.Mock).mockResolvedValue({ versionNo: 1 });
      (prisma.policyVersion.create as jest.Mock).mockResolvedValue(createdVersion);
      (prisma.policy.update as jest.Mock).mockResolvedValue({ id: policyId });
  ...
  describe('saveCurrentDocx', () => {
    it('updates the existing current version without creating a new version', async () => {
      ...
      (prisma.policyVersion.findUnique as jest.Mock).mockResolvedValue({ id: versionId, policyId });
      (prisma.policyVersion.update as jest.Mock).mockResolvedValue(updatedVersion);
      (prisma.policy.update as jest.Mock).mockResolvedValue({ id: policyId });
      (prisma.auditLog.create as jest.Mock).mockResolvedValue({ id: 'audit-1' });
